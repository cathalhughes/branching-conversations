...
export const EditingSessionSchema =
  SchemaFactory.createForClass(EditingSessionModel);

// TTL Index - automatically remove inactive sessions after 24 hours
EditingSessionSchema.index(
  { lastActivityAt: 1 },
...
EditingSessionSchema.statics.findActiveByCanvas = function (
  canvasId: Types.ObjectId,
) {
  return this.find({
    canvasId,
...
EditingSessionSchema.statics.findActiveByTarget = function (
  editingTarget: Types.ObjectId,
) {
  return this.find({
    editingTarget,
...
EditingSessionSchema.statics.findByUser = function (
  userId: Types.ObjectId,
  isActive = true,
) {
...
EditingSessionSchema.statics.hasActiveLock = async function (
  editingTarget: Types.ObjectId,
