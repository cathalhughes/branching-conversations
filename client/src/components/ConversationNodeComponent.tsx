...
const ConversationNodeComponent = observer((props: NodeProps) => {
  ...
  return (
    <div 
      className="bg-white border-2 rounded-lg p-4 min-w-[300px] max-w-[500px] shadow-md"
      style={{ 
        borderColor: node.lastEditedBy?.color || '#d1d5db',
        borderLeftWidth: '4px',
        borderLeftColor: node.lastEditedBy?.color || '#d1d5db'
      }}
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {node.lastEditedBy && (
            <div 
              className="flex items-center gap-1 text-xs px-2 py-1 rounded text-white font-medium"
              style={{ backgroundColor: node.lastEditedBy.color }}
              title={`Last edited by ${node.lastEditedBy.userName}`}
            >
              <div className="w-5 h-5 rounded-full bg-white bg-opacity-20 flex items-center justify-center text-xs font-bold">
                {getInitials(node.lastEditedBy.userName)}
              </div>
              <span>{node.lastEditedBy.userName}</span>
            </div>
          )}
        </div>
        <div className="flex items-center gap-2">
